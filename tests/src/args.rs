...
impl CliArguments {
    pub fn should_run(&self, stage: impl Into<TestStages>) -> bool {
        ...
        if CACHED.load(Ordering::Relaxed) == 0xFF {
            ...
            if self.stages.is_empty() {
                ...
                if stages.contains(TestStages::PAGED) {
        ...
        let stages = TestStages::from_bits(CACHED.load(Ordering::Relaxed)).unwrap();
        stages.intersects(stage.into())
    }
}
