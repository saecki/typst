...
fn main() {
    ...
    Command::new("git")
        ...
        .run()
        .exit_on_failure("work tree is dirty");
    ...
    for rev in revs.iter().take(3) {
        ...
        // If the test suite passes, there aren't any more missing live outputs.
        if status.success() {
            break;
        }

        // Even a failing test suite could produce all missing live outputs.
...
fn test_name(hash_path: &str) -> &str {
    let filename_start = hash_path.rfind('/').unwrap() + 1;
    let extension_start = hash_path.rfind('.').unwrap();
    &hash_path[filename_start..extension_start]
}
