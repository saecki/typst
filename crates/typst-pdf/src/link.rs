...
pub(crate) fn handle_link(
    ...
    if tags::disabled(gc) {
        if gc.tags.in_tiling && gc.config.is_pdf_ua() {
            let validator = gc.config.standards.config.validator().as_str();
            bail!(
                Span::detached(),
    ...
    let join_annotations = gc.config.is_pdf_ua();
